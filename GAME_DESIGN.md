...
- Minimum word length: 3, maximum 40 (raised from 12, 2026-07-17 — roomy on purpose: no real word gets there, but creative nonsense might; a big word keeps its whole length bonus). Minimum overlap: 2. **Minimum extension: 2** (added 2026-07-17) — your word must add at least two letters of its own past the overlap. Adding an S is cheap: `ram` → `rams` is not a play, and neither is swallowing the tip whole (`planet` → `net`). Gripping the full previous word is still legal when you outgrow it by two: `ultra` → `ultramarine`, overlap 5, glorious. A grip too deep to extend falls back to the deepest shallower one that fits.
...
- Openers (the match's first word, and a fresh chain after a snap) have overlap 0, so the same formula pays **1 point per letter** (changed 2026-07-18 — openers used to be worth 0).
...
**The snap (added 2026-07-16):** if both players pass on the same word — two passes in a row, with no word or challenge between them — the chain **snaps** instead of ping-ponging passes until someone runs out of lives. Both passes still cost their life as usual. Whoever is on move after the second pass **starts a fresh chain**: any word, 3+ letters, no overlap required, worth 1 point per letter (same as the match opener). The words behind the break are settled — the sealed tip can't be challenged anymore (each player already had the chance to flag it instead of passing). Used words stay used across the break; the total word count (chain limit, last call) also carries across. If a fresh opener is challenged and REJECTED, the break re-opens and the challenger starts the fresh chain instead. In the ledger the break is a **dotted seam** across the diagonal — the staircase keeps its position (the fresh word starts one step past the whole sealed word), the rail thread is cut at the seam (the seam's ends fade out — no hard stops), and the fresh opener's meta reads "fresh chain". State: `breaks: number[]` on `MatchState` records where each fresh chain starts; a break equal to `chain.length` is pending (`isChainBroken`), and `passStreak` counts consecutive passes. Copy: the player **starts** a fresh chain — never "deals" one (Matt's call, 2026-07-16: it's not a card game; keep card verbs out even though the table-talk voice stays).
...
- The chain is a pure staircase on a virtual canvas: each word's row is offset so its head letters sit under the tail letters it grips. No wrapping, no cropping. Words up to ~13 tiles fit the viewport whole; longer ones (cap 40, 2026-07-17) get the **caret-follow rule** — the camera keeps riding so the business end stays in frame (see The parked camera). The bot never plays past 12 (`BOT_MAX_WORD_LENGTH`), so only humans summon the monsters.
...
- **The parked camera**: on the first letter, the view slides so the gripped letters pin to the left edge (x=14); the previous word's spent letters bleed off-screen. Your row head sits at the edge with the full width ahead — up to ~13 tiles fit with room for the caret. **Caret-follow (2026-07-17):** past that, the rail keeps sliding so the caret pins near the right edge; the spent grip dissolves off-left under a soft fade, and backspace slides it back. The same rule tail-anchors the ghost fan on a long tip word (seeds + challenge flag stay in frame), and an incoming long word's type-in reveal rides the camera the same way, ending exactly at the fan's park. Scroll-back stays head-pinned — a word is read from its start; the locked row's meta tucks under the head when the tail runs off-frame, and the detail card is the one place a monster is readable whole (tiles fold there). Elsewhere (final scoreboard, invite share card + landing) long words ride a one-line swipeable rail under edge fades. Backspacing to empty un-parks. "◀ N words back" chip is the escape to the full chain. Under prefers-reduced-motion the park is a cut, not a glide.
