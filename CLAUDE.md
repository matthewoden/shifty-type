...
Async two-player word game (plus solo vs the llama bot). Renamed 2026-07 from "Word Chain" to "Spell Llama", then to **Shifty Type** — the name plays the game's own move (SHIFTY → TYPE, overlap TY; fused: SHIFTYPE) and means the sneak across the table. The bot remains Lloyd the llama. The voice is silly card-table, never crime (no vault/heist/thieves/stash/hideout) — but as of 2026-07 the furniture is never named in player-facing copy: no "table", no "seat"/"sit down" (players "jump in" or "join"; shaking, sliding notes, "in play" remain; internal names like Seat/seat link stay). The other player is always a **friend** — never "rival"/"rivals" (or opponent-hostile framing) anywhere in user-facing copy, notifications included. Deployed at **https://shifty-type.heymatthewoden.workers.dev** (Worker renamed 2026-07-12; the old word-chain Worker and its matches were deleted). Mobile-first web app.
