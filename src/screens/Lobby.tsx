...
const PILL =
    "text-[12px] font-extrabold rounded-full px-2.5 py-1 whitespace-nowrap";
const yourTurnPill = (
    <span
        className={`${PILL} bg-p1 text-white shadow-[0_3px_0_var(--color-p1-lip)] pt-1.5`}
    >
        Your turn
    </span>
);
const theirTurnPill = (
    <span className={`${PILL} bg-board text-dim `}>Their turn</span>
);
function resultPill(winner: PlayerId | null, you: PlayerId): ReactNode {
    if (winner === you)
        return (
            <span className={`${PILL} bg-[#E9F7EC] text-[#2A9D4E]`}>
                You won
            </span>
        );
    if (winner)
        return <span className={`${PILL} bg-board text-dim`}>They won</span>;
        ...
        <>
            ...
            <span className="flex-1 min-w-0">
                <span className="flex items-center gap-1.5 min-w-0">
                    <span className="font-extrabold text-ink-strong truncate">
                        {name}
                    </span>
                    {live && !editing && (
    ...
    const info = (
        <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <span className="min-w-0">
                <span className="block font-extrabold text-ink-strong truncate">
                    Waiting to join
                </span>
    ...
    return (
        <section className="flex flex-col gap-2.5">
            <div className="flex items-center gap-2 px-1">
                <span className="text-label font-extrabold tracking-wider uppercase text-dim">
                    {title}
                </span>
                <span className="text-label font-extrabold text-dim">
                    {count}
                </span>
            </div>
            {children}
...
export function Lobby({
    ...
    useEffect(() => {
        ...
        void (async () => {
            const r = await api.matchesSummary(
                seats.map((s) => ({ code: s.code, token: s.auth.token })),
            );
            ...
            const unstarted = (s: MatchSummary) =>
                s.awaitingOpponent && !s.openingWord;
            r.gone.forEach(removeMatchAuth); // matches deleted after 60 days
            r.summaries
                .filter(unstarted)
                .forEach((s) => removeMatchAuth(s.code));
    ...
    const byRecent = (a: MatchSummary, b: MatchSummary) =>
        (b.lastMoveAt ?? 0) - (a.lastMoveAt ?? 0);
    const yourMove = summaries
        .filter((s) => duelBucket(s) === "yourMove")
        .sort(byRecent);
    const theirMove = summaries
        .filter((s) => duelBucket(s) === "theirMove")
        .sort(byRecent);
    const pending = summaries
        .filter((s) => duelBucket(s) === "pending")
        .sort(byRecent);
    const finished = summaries
        .filter((s) => duelBucket(s) === "finished")
        .sort(byRecent);
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-8 max-w-[430px] w-full mx-auto flex flex-col gap-6 pt-2">
                {empty ? (
                    <div className="flex-1 flex flex-col items-center justify-center gap-2 text-center py-20">
                        <p className="font-extrabold text-ink-strong text-lg">
                            No games yet
                        </p>
                        <p className="font-semibold text-dim text-body max-w-[15rem]">
                            Challenge a friend or play a llama, and your games
                            will gather here.
                        </p>
                    </div>
                ) : (
                    <>
                        ...
                        {finishedCount > 0 && (
                            <div className="flex flex-col gap-2.5">
                                ...
                                >
                                    {showFinished ? "Hide" : "Finished games"} (
                                    {finishedCount}) {showFinished ? "▲" : "▸"}
                                </Button>
                                {showFinished && (
                                    <div className="flex flex-col gap-2.5">
