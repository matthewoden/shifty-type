import { STARTING_LIVES, chainLimitOf, opponentOf, type MatchState, type PlayerId } from '../game'
  ...
  return (
    <span className="inline-flex gap-1 mt-1">
      {Array.from({ length: STARTING_LIVES }, (_, i) => (
        <span
          key={i}
          className={`w-2.5 h-2.5 rounded-full ${i < lives ? alive : 'bg-board-lo shadow-[0_2px_0_#DDD8CE]'}`}
        />
      ))}
    </span>
  )
